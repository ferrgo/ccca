...
describe('Creating Order', () => {
    ...
    describe('Adding items', () => {
        test('Should create an Order with 1 item', () => {
            const order = new Order('935.411.347-80');
            order.addItem(new Product(1, 'Books', 'Clean Architecture', 50), 1);
        ...
        test('Should create an Order with 3 items', () => {
            const order = new Order('935.411.347-80');
            order.addItem(new Product(1, 'Books', 'Clean Architecture', 50), 1);
            order.addItem(new Product(2, 'Books', 'Clean Code', 70), 1);
            order.addItem(new Product(3, 'Food', 'Coffee Beans 1kg', 10), 4);
            const itemCount = order.getItemCount();
    ...
    describe('Applying coupons', () => {
        ...
        test('Should discount 0 from order if no coupon applied', () => {
            const order = new Order('935.411.347-80');
            order.addItem(new Product(1, 'Books', 'Clean Architecture', 50), 9);
            order.addItem(new Product(2, 'Books', 'Clean Code', 70), 5);
            order.addItem(new Product(3, 'Food', 'Coffee Beans 1kg', 10), 4);
            order.addItem(new Product(3, 'Food', 'Coffee Beans 3kg', 10), 2);
            order.addItem(new Product(3, 'Home Office', 'Table Plant', 2), 100);
